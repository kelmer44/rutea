 ...
 */
public abstract class GenericDaoOrmLiteImpl<E, PK extends Serializable> extends AbstractDaoOrmLiteImpl implements GenericDao<E, PK> {
    /**
     * Logging
     */
    private static final String LOG_TAG = GenericDaoOrmLiteImpl.class.getSimpleName();
	/**
    ...
    public GenericDaoOrmLiteImpl(final Context context) {
        ...
        Log.d(LOG_TAG, "Creating DAO for " + entityClass.getSimpleName() + " from " + getClass().getSimpleName());

        OrmLiteSqliteOpenHelper helper = OpenHelperManager.getHelper(context, DatabaseHelper.class);
        try {
            dao = helper.getDao(entityClass);
        } catch (SQLException e) {
            throw new RuntimeException("Could not instantiate a DAO for class " + entityClass.getName(), e);
        }
        //helper.close();
    ...
    protected DatabaseHelper<E, PK> getDatabaseHelper() {
        DatabaseHelper<E, PK> databaseHelper = (DatabaseHelper<E, PK>) OpenHelperManager.getHelper(getContext(), DatabaseHelper.class);
